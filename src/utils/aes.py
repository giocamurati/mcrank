...
class AES(object):
    ...
    def aes_round(self, state, roundKey):
        ...
        subbytesoutput = [state[i*4+j] for j in range(4) for i in range(4)]
        state = self.shiftRows(state, False)
        state = self.mixColumns(state, False)
        state = self.addRoundKey(state, roundKey)
        return state, subbytesoutput
    ...
    def aes_main(self, state, expandedKey, nbrRounds):
        ...
        i = 1
        while i < nbrRounds:
            state, subbytesoutput = self.aes_round(state,
                                   self.createRoundKey(expandedKey, 16*i))
            states.append(subbytesoutput)
            i += 1
        state = self.subBytes(state, False)
        state = self.shiftRows(state, False)
    ...
    def encrypt(self, iput, key, size):
        ...
        block, states = self.aes_main(block, expandedKey, nbrRounds)
